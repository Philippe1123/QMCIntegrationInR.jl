...
#    Mat[:,ct,1]=map.(funTransform,Lattice[mp-1],BigBoxInterval)
   ...
   cumsum=sum((Mat_full[:,mp,Nshift] .> SmallBoxInterval)  .| (Mat_full[:,mp,Nshift] .< -SmallBoxInterval))
...
plot([SmallBoxInterval,SmallBoxInterval],[-SmallBoxInterval,SmallBoxInterval],"k")
...
strtitle=string("Dimensioning number of samples is ",N_dim, "\n"," Actual number of samples  is ", N,"\n","big box is [- ", round(BigBoxInterval,digits=2)," ", round(BigBoxInterval,digits=2), "] ", "small box is [- ", round(SmallBoxInterval,digits=2)," ", round(SmallBoxInterval,digits=2), "]")
title(strtitle)
